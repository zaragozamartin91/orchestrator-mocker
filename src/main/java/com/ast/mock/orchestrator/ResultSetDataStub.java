...
public class ResultSetDataStub implements IResultSetData {
	...
	public static ResultSetDataStub buildFull(int rowSize, boolean lenientResultSetRow, Object... items) {
		if (items.length == 0) {
			throw new IllegalArgumentException("No se pasaron valores!");
		}
		if (items.length % rowSize != 0) {
			throw new IllegalArgumentException("El tamano de fila es incorrecto!");
		}
	...
	public IResultSetRow getRow(int i) {
		if (i <= 0) {
			throw new IllegalArgumentException("El indice comienza en 1");
		}
		return rows.get(i - 1);
	}
